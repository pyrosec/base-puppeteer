...
class BasePuppeteer {
    static async initialize({ headless, noSandbox, logger, session, proxyServer } = {}) {
        ...
        if (proxyParams) {
            ...
            const result = proxyParams.cycleOrBuy === 'random' ? pickRandom(Object.values((await proxy6.getproxy({})).list).filter((v) => v.country === proxyParams.country && (proxyParams.ipv4OrIpv6 === 'ipv4' || v.version === '6'))) : Object.values((await proxy6.buy({
        ...
        const parsedProxyServer = url_1.default.parse(proxyServer);
        const { hostname, port, auth, protocol } = parsedProxyServer;
        const args = proxyServer
            ? ["--proxy-server=" + url_1.default.format({ hostname, port, protocol })]
            : [];
