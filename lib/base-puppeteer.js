...
class BasePuppeteer {
    static async initialize({ headless, noSandbox, logger, session, proxyServer } = {}) {
        ...
        if (proxyParams) {
            ...
            const result = proxyParams.cycleOrBuy === 'random' ? pickRandom(Object.values((await proxy6.getproxy({})).list).filter((v) => v.type === 'http' && (v.country === proxyParams.country && (proxyParams.ipv4OrIpv6 === 'ipv4' || v.version === '6')))) : Object.values((await proxy6.buy({
        ...
        const args = proxyServer
            ? ["--proxy-server=" + protocol + '//' + (net_1.default.isIPv6(hostname) ? `[${hostname}]` : hostname) + ':' + port]
            : [];
